		...
		socket.on("display message", function (message) {
			if(message.startsWith("Adam:")){
				...
				var messageIndex = localStorage.getItem("messageCount-A");
				
					var messageStorageName = "message-A" + messageIndex;
			 		...
			 		localStorage.setItem("messageCount-A",++messageIndex);

				if(messageIndex >= totalAnswers){
					localStorage.setItem("totalAnswersOverHundred", 1);	
					localStorage.setItem("messageCount-A", 0);	
				}
		...
		function showMessages(currentDisplayByInput) {
				...
				displayIndex = localStorage.getItem("showIndex");
					...
					for (var i = 0; i<displayIDs.length; i++){
						...
						var messageStorageName = "message-A" + displayIndex;
